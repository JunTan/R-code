...
```{r}
# use final_data_no_AL for predictor base on the voting percent statistic from 04, 08, 12. Voting percent statistic from 16 is used for Testing
chooseTest_dem = c(1,2,10)
votingTest_dem = final_data[chooseTest_dem]

chooseTest_gop = c(1,2,8)
votingTest_gop = final_data[chooseTest_gop]

chooseTrain_dem = c(1,2,18,6,14,10)
votingTrain_dem = final_data[ , chooseTrain_dem ]

chooseTrain_gop = c(1,2,17,4,12,8)
votingTrain_gop = final_data[ , chooseTrain_gop]
```

## Training for 2016 democrate voting percentage

Create a 3-column matrix called `folds` that contains indices for partitioning the `tweetTrain` data frame into 3 folds.
...
v = 3
folds_dem = matrix(permuteIndices, ncol = 3)
```
...
for (i in 1:3) {
  ...
  for (j in 1:length(cps)) {
    tree = rpart(vote_percent_dem16 ~ vote_percent_dem04+vote_percent_dem08+vote_percent_dem12,
