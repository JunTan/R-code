...
```{r}
# clean the final dataframe to make the data with all none-na stats
df_names = names(big_data)
filter = sapply(big_data[1], function(x) !is.na(x))
for (name in df_names[2:(length(df_names))]){
  inter_filter = sapply(big_data[name], function(x) !is.na(x))
  filter = filter & inter_filter
}
final_data = big_data[filter,]
```
