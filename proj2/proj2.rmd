...
paste("classification rate of the testing set and training set:", classRate1)
```
...
pred_plot = ggplot(data = pred_df, aes(pred_df$testPreds2)) + geom_bar() + labs(x = "changes from 2012 to 2016", title = "Prediction changes from 2012 to 2016")
pred_plot

actual_change_df = data.frame(Change12_16$change_type)
acutal_plot = ggplot(data = actual_change_df, aes(actual_change_df$Change12_16.change_type)) + geom_bar() + labs(x = "changes from 2012 to 2016",title = "Actual changes from 2012 to 2016")
acutal_plot
```
